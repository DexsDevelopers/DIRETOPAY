...
$stmt = $pdo->prepare("SELECT balance, commission_rate, pix_key, status, is_demo, is_admin, full_name, email, referral_token, withdraw_method, crypto_address, crypto_network, api_key, whatsapp, utmify_api_token, seven_k_id, preferred_nominal FROM users WHERE id = ?");
$stmt->execute([$userId]);
$user = $stmt->fetch();
