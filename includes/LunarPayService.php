<?php
/**
 * LunarPayService - Integração com API LunarPay
 * Documentação: https://lunarpay.com.br/docs
 * 
 * Funcionalidades:
...
class LunarPayService
{
    private static function getClientId(): string
    {
        ...
        $stmt->execute(['lunarpay_client_id']);
    ...
    private static function getClientSecret(): string
    {
        ...
        $stmt->execute(['lunarpay_client_secret']);
        return (string)($stmt->fetchColumn() ?: '');
    }
    ...
    private static function getWebhookSecret(): string
    {
        ...
        $stmt->execute(['lunarpay_webhook_secret']);
        return (string)($stmt->fetchColumn() ?: '');
    }
    ...
    {
        if (!self::isEnabled()) {
            return ['ok' => false, 'error' => 'LunarPay não habilitado'];
        }
        ...
        if (!$clientId || !$clientSecret) {
            return ['ok' => false, 'error' => 'Credenciais LunarPay não configuradas'];
        }

        $url = 'https://api.lunarpay.com.br' . $endpoint;
