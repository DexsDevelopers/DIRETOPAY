...
const NavLabel = React.memo(({ label, isDark }) => {
  return (
    <p
      className={cn(
        "px-3 pt-5 pb-1.5 text-[9px] font-black uppercase tracking-[0.12em]",
...
const NavItem = React.memo(
  ({ item, location, onTabChange, onClose, isDark }) => {
    const isActive =
    ...
    return (
      ...
      >
        ...
        <span
          className={cn(
            "shrink-0 w-[18px] h-[18px] flex items-center justify-center",
            isActive
              ? "text-emerald-500"
              : isDark
...
const NavSubItem = React.memo(
  ({ item, location, onTabChange, onClose, isDark }) => {
    ...
    const handleClick = useCallback(() => {
      if (!item.external) onTabChange(item.id);
      if (window.innerWidth < 1024) onClose();
    }, [item.id, item.external, onTabChange, onClose]);
    ...
    return (
      <div className="mb-0.5">
        <button
          ...
          className={cn(
            "flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors duration-150 group w-full will-change-transform",
            hasActive
              ? isDark
  ...
  return (
    <>
      ...
      >
        ...
        {/* Navigation */}
        <nav
          className="flex-1 px-2.5 py-2 overflow-y-auto custom-scrollbar"
          ...
          {navSections.map((section, si) => (
            <div key={si}>
              {section.label && (
                <p
                  className={cn(
                    "px-3 pt-4 pb-1.5 text-[9.5px] font-black uppercase tracking-[0.1em]",
                    isDark ? "text-gray-600" : "text-gray-400",
                  )}
                >
                  {section.label}
        ...
        <div className={cn("border-t p-3 shrink-0", border)}>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p
                className={cn(
                  "text-[12.5px] font-bold truncate leading-tight",
          ...
          <button
            ...
            className={cn(
              "w-full flex items-center gap-2.5 px-2 py-2 rounded-lg text-[12.5px] font-medium transition-all",
              isDark
