...
import { Lock, Mail, ArrowRight, Zap, KeyRound, Check, ShieldCheck, Users, Eye, EyeOff } from 'lucide-react';
import { CustomCursor, NoiseOverlay, MagneticButton, GlowInput } from '../components/AnimatedUI';
        ...
        <div className="min-h-screen bg-[#060809] text-white flex flex-col items-center justify-center overflow-hidden relative px-4">
            ...
            <motion.div
                ...
                <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-2xl p-8"
                    style={{ boxShadow: '0 0 0 1px rgba(255,255,255,0.05), 0 40px 100px rgba(0,0,0,0.55), 0 0 80px rgba(16,185,129,0.05)' }}>

                    {/* Header */}
                    <div className="mb-7">
                        <AnimatePresence mode="wait">
                            <motion.div key={mustResetPassword ? 'reset-h' : 'login-h'}
                                initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }}
                                transition={{ duration: 0.25 }}>
                                <h1 className="text-[22px] font-bold tracking-tight text-white">
                                    {mustResetPassword ? 'Criar nova senha' : 'Bem-vindo de volta'}
                                </h1>
                                <p className="text-[13px] text-gray-500 mt-1">
                                    {mustResetPassword ? 'Defina uma senha segura para continuar' : 'Entre na sua conta DiretoPay'}
                                </p>
                            </motion.div>
                        </AnimatePresence>
                    </div>
                    ...
                    <AnimatePresence>
                        {resetSuccess && (
                            <motion.div key="ok" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                                ...
                                <Check size={13} className="shrink-0" /> Senha atualizada! Faça login agora.
                            </motion.div>
                        )}
                    </AnimatePresence>
                    ...
                    <AnimatePresence mode="wait">
                        {mustResetPassword ? (
                            <motion.form key="reset" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }}
                                transition={{ duration: 0.3 }} onSubmit={handleResetPassword} className="space-y-4">
                                ...
                                <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[12px] px-4 py-3 rounded-xl">
                                    <KeyRound size={13} className="shrink-0" /> Crie uma nova senha para continuar.
                                </div>

                                <div>
                                    <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-widest mb-2">Nova senha</label>
                                ...
                                <div>
                                    <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-widest mb-2">Confirmar senha</label>
                                ...
                                <MagneticButton type="submit" disabled={loading} strength={0.2}
                                    ...
                                    {loading ? <><span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />Salvando...</> : <>Definir senha <ArrowRight size={15} /></>}
                                </MagneticButton>
                            ...
                            <motion.form key="login" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }}
                                ...
                                {/* Email */}
                                <div>
                                    <label className="block text-[11px] font-semibold text-gray-500 uppercase tracking-widest mb-2">E-mail</label>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        <label className="text-[11px] font-semibold text-gray-500 uppercase tracking-widest">Senha</label>
                                        <Link to="/forgot-password" className="text-[11px] text-emerald-500 hover:text-emerald-400 transition-colors">
                                            Esqueci a senha
                                        </Link>
                                    </div>
                                ...
                                {/* CTA */}
                                <MagneticButton type="submit" disabled={loading} strength={0.25}
                                    ...
                                    {loading
                                        ? <><span className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" />Entrando...</>
