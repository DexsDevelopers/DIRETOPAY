...
export default function RegisterPage() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!emailDomain || !allowedDomains.includes(emailDomain)) {
      setError(
        "Use um e-mail de provedor confiável (Gmail, Outlook, Hotmail, Yahoo, iCloud, etc). E-mails temporários não são permitidos.",
      );
      setLoading(false);
      setStep(1);
      return;
  ...
  const inp =
    "w-full rounded-xl py-3 pl-10 pr-4 text-[13px] border outline-none transition-all bg-white/5 border-white/10 text-white placeholder:text-gray-600 focus:border-emerald-500/50";
  ...
  const NOMINALS = [
    {
      ...
      desc: "Nichos sensíveis ou com alta taxa de chargeback.",
      min: "R$ 25,00",
      icon: ShieldAlert,
    },
    {
      ...
      desc: "Produtos baratos sem risco de estorno.",
      min: "R$ 5,00",
      icon: Briefcase,
    },
    {
      ...
      desc: "Ticket médio com repasse automático.",
      min: "R$ 10,00",
      icon: Zap,
    },
  ];
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            <div
              className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${step >= 1 ? "bg-emerald-500 text-white" : "bg-white/10 text-gray-500"}`}
            >
              1
            </div>
            <div
              className={`w-8 h-px ${step >= 2 ? "bg-emerald-500" : "bg-white/10"}`}
            />
            <div
              className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${step >= 2 ? "bg-emerald-500 text-white" : "bg-white/10 text-gray-500"}`}
            >
              2
            </div>
          </div>
        ...
        >
          <AnimatePresence mode="wait">
            {step === 1 ? (
              <motion.form
                ...
                onSubmit={(e) => {
                  ...
                  if (!domain || !allowedDomains.includes(domain)) {
                    setError(
                      "Use um e-mail de provedor confiável (Gmail, Outlook, Yahoo, iCloud, etc).",
                    );
                    return;
                  }
                  setStep(2);
              ...
              >
                {error && (
                  <p className="text-[12px] text-red-400 bg-red-500/10 border border-red-500/20 p-3 rounded-xl">
                    {error}
                  </p>
                )}
              ...
              >
                  ...
                  <div className="space-y-2">
                    {NOMINALS.map(
                      ({
                        ...
                        <div
                          key={key}
                          onClick={() => handleSelectNominal(key)}
                        ...
                        >
                          <div
                            className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${preferredNominal === key ? "bg-emerald-500/20" : "bg-white/5"}`}
                          >
                            <Icon
                              size={13}
                              className={
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              ...
                              <span
                                className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${tagColor}`}
                              >
                                {tag}
                              </span>
                            </div>
                          ...
                          <div
                            className={`w-4 h-4 rounded-full border-2 shrink-0 mt-1 transition-all ${preferredNominal === key ? "border-emerald-500 bg-emerald-500" : "border-gray-600"}`}
                          />
                        </div>
                      ),
                    )}
                      ...
                      {[
                        {
                          ...
                          icon: PiggyBank,
                          desc: "Saque manual quando quiser",
                        },
                        {
                          key: "auto_direct",
                          label: "Por venda",
                          icon: Zap,
                          desc: "PIX automático a cada venda",
                        },
                      ].map(({ key, label, icon: Icon, desc }) => (
                        <div
                          key={key}
                          onClick={() => setWithdrawPreference(key)}
                        ...
                        >
                          <Icon
                            size={16}
                            className={`mx-auto mb-1.5 ${withdrawPreference === key ? "text-emerald-400" : "text-gray-500"}`}
                          />
                ...
                {preferredNominal === "nominal3" && (
                  <div className="bg-blue-500/5 border border-blue-500/20 text-blue-400 rounded-xl p-3 flex gap-2 items-center text-[12px]">
                    <Zap size={14} className="shrink-0" /> Rota N3: saque
                    automático já ativo por padrão.
                  </div>
                )}
