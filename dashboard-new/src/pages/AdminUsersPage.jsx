...
export default function AdminUsersPage() {
  ...
  const fetchAdminData = async () => {
    try {
      const res = await fetch(
        `../get_admin_data.php?search=${encodeURIComponent(search)}&status_filter=${statusFilter}&order_by=${orderBy}`,
      );
  ...
  /* ── Summary mini-stats ── */
  const pending =
    adminData?.stats?.pending_users ??
    users.filter((u) => u.status === "pending").length;
  const approved =
    adminData?.stats?.approved_users ??
    users.filter((u) => u.status === "approved").length;
  const blocked =
    adminData?.stats?.blocked_users ??
    ...
    <div className="space-y-8 p-6 lg:p-10 max-w-[1600px] mx-auto animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium">
            Aprovações, saldos, permissões e saques.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          {
            label: "Total",
            value: fmtN(adminData?.stats?.total_users ?? users.length),
            color: "text-gray-900",
            icon: <Users size={16} />,
          },
          {
              ...
              >
                ...
                <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      {user.is_demo === 1 && (
                        <span className="text-[7px] bg-emerald-500/15 text-emerald-500 px-1.5 py-0.5 rounded font-black shrink-0 tracking-wider">
                          DEMO
                        </span>
                      )}
                    </div>
                ...
                <div className="px-4 pb-3 grid grid-cols-4 gap-2">
                  <div className="bg-gray-50 rounded-xl px-2 py-2 text-center flex flex-col items-center justify-center">
                    <span className="text-[8px] text-gray-400 font-bold uppercase block mb-0.5">
                      Faturamento
                    </span>
                    ...
                    <div className="flex items-center justify-center gap-0.5">
                      ...
                      <input
                        ...
                        onBlur={(e) =>
                          ...
                          handleAction("update_user_field", {
                            user_id: user.id,
                            field: "balance",
                  ...
                  <div className="bg-gray-50 rounded-xl px-2 py-2 text-center flex flex-col items-center justify-center">
                    ...
                    <span
                      className={cn(
                        "text-[9px] font-black",
                        user.withdraw_method === "btc"
                          ? "text-orange-400"
                          : user.withdraw_method === "usdt"
                            ? "text-teal-400"
                            : "text-emerald-500",
                      )}
                    >
                  ...
                  <div className="bg-gray-50 rounded-xl px-2 py-2 text-center flex flex-col items-center justify-center">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleAction("update_user_field", {
                          user_id: user.id,
                          field: "preferred_nominal",
                          value: e.target.value,
                        })
                      }
                      className="bg-transparent text-[9px] font-black text-emerald-500 focus:outline-none cursor-pointer text-center"
                    >
                      <option value="nominal1" style={{ color: "#000" }}>
                        Nominal 1
                      </option>
                      <option value="nominal2" style={{ color: "#000" }}>
                        Nominal 2
                      </option>
                      <option value="nominal3" style={{ color: "#000" }}>
                        Nominal 3
                      </option>
                      <option value="nominal4" style={{ color: "#000" }}>
                        Nominal 4
                      </option>
                    </select>
                  </div>
                ...
                <div className="px-4 pb-2 flex items-center gap-2">
                  <span className="text-[8px] text-gray-400 font-bold uppercase block flex-shrink-0">
                    Ezzy:
                  </span>
                  <input
                    type="text"
                    placeholder="Ex: OWEMPAY"
                    defaultValue={user.ezzy_acquirer || ""}
                    onBlur={(e) =>
                      e.target.value !== (user.ezzy_acquirer || "") &&
                      handleAction("update_user_field", {
                        user_id: user.id,
                        field: "ezzy_acquirer",
                        value: e.target.value,
                    ...
                    className="bg-gray-50 border border-gray-200 rounded-lg px-2 py-1 text-xs font-mono text-gray-700 focus:outline-none flex-1"
                  />
                </div>
                ...
                <div className="px-4 pb-3 flex items-center gap-2 flex-wrap">
                  {!user.withdraw_method || user.withdraw_method === "pix"
                    ? user.pix_key && (
                        <span className="text-[11px] text-gray-500 font-mono bg-gray-50 px-2.5 py-1 rounded-lg truncate max-w-[60%]">
                          {user.pix_key}
                        </span>
                      )
                    : user.crypto_address && (
                  ...
                  {user.whatsapp && (
                    <div className="flex items-center gap-1 ml-auto shrink-0">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="flex items-center gap-1 text-[11px] font-bold text-green-400 bg-green-500/5 px-2.5 py-1 rounded-lg"
                      >
                ...
                <div className="px-4 pb-4 flex items-center gap-1.5">
                  <button
                    onClick={() => {
                      if (confirm(`Resetar senha de ${user.full_name}?`))
                        handleAction("reset_user_password", {
                          user_id: user.id,
                        });
                    }}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-amber-500/10 rounded-xl text-amber-400 text-[10px] font-bold active:scale-95 transition-transform"
                    title="Resetar Senha"
                  >
                  ...
                  <button
                    onClick={() =>
                      setShowFakeWithdrawModal({
                        userId: user.id,
                        name: user.full_name,
                        pix: user.pix_key,
                      })
                    }
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-gray-100 rounded-xl text-gray-500 text-[10px] font-bold active:scale-95 transition-transform"
                    title="Saque Fake"
                  >
                    <Zap size={12} /> Saque
                  </button>
                  <button
                    onClick={() =>
                      handleAction("update_user_field", {
                        user_id: user.id,
                        field: "is_demo",
                        value: user.is_demo ? 0 : 1,
                      })
                    }
                    className={cn(
                      "flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-[10px] font-bold active:scale-95 transition-transform",
                      user.is_demo
                  ...
                  >
                    {user.is_demo ? <Eye size={12} /> : <EyeOff size={12} />}{" "}
                    Demo
                  </button>
                  {user.status !== "approved" && (
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-emerald-500/10 rounded-xl text-emerald-400 text-[10px] font-bold active:scale-95 transition-transform"
                    >
                      <CheckCircle size={12} />
                    </button>
                  )}
          ...
          <table className="w-full min-w-[900px]">
            <thead>
              <tr className="text-left border-b border-gray-100">
                <th className="p-6 pl-10 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                  ID / Usuário
                </th>
                ...
                <th className="p-6 pr-10 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">
                  Ações
                </th>
              </tr>
            </thead>
                ...
                >
                  <td className="p-6 pl-10">
                    <div className="flex flex-col">
                      ...
                      {user.is_demo === 1 && (
                        <span className="text-[9px] bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-2 py-0.5 rounded-full w-fit mt-1 font-black">
                          DEMO
                        </span>
                      )}
                    </div>
                  ...
                  <td className="p-6">
                    <div className="flex flex-col gap-1">
                      ...
                      user.withdraw_method === "pix" ? (
                        <div className="flex items-center gap-2">
                          <input
                            defaultValue={user.pix_key}
                            onBlur={(e) =>
                              e.target.value !== user.pix_key &&
                              handleAction("update_user_field", {
                                user_id: user.id,
                                field: "pix_key",
                                value: e.target.value,
                        ...
                        <span
                          className="text-[11px] text-gray-500 font-mono truncate max-w-[180px]"
                          title={user.crypto_address}
                        >
                          {user.crypto_address || (
                      ...
                      {user.whatsapp ? (
                        <div className="flex items-center gap-1">
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="flex items-center gap-1.5 text-[11px] font-bold text-green-400 hover:text-green-300 transition-colors"
                          >
                            {WA_SVG} {fmtWA(user.whatsapp)}
                          </a>
                          <CopyWA number={user.whatsapp} />
                  ...
                  <td className="p-6">
                    <div className="flex justify-center">
                      ...
                      >
                        ...
                        <option value="nominal4">Nominal 4</option>
                      </select>
                    </div>
                  </td>
                  <td className="p-6">
                    <div className="flex justify-center">
                      <input
                        ...
                        className="bg-gray-50 border border-gray-200 rounded-xl py-2 px-3 text-xs font-bold text-gray-700 focus:outline-none text-center w-32"
                      />
                    </div>
                  </td>
                  <td className="p-6">
                  ...
                  <td className="p-6 pr-10">
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        ...
                        className="p-2.5 bg-gray-100 rounded-xl text-gray-400 hover:bg-emerald-500/10 hover:text-emerald-500 transition-all border border-gray-200"
                        title="Saque Fake"
                      >
                        <Zap size={16} />
                      </button>
                      <button
                        ...
                        className={cn(
                          ...
                          user.is_demo
                            ...
                            : "bg-gray-100 text-gray-400 border-gray-200 hover:bg-gray-200 hover:text-gray-900",
                        )}
                        title={user.is_demo ? "Desativar Demo" : "Ativar Demo"}
                      >
                        {user.is_demo ? (
        ...
        <div className="bg-white border border-gray-100 shadow-sm rounded-[40px] overflow-hidden animate-in slide-in-from-bottom duration-500">
          <div className="p-8 border-b border-gray-100 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 font-medium mt-0.5">
                {withdrawals.length} pendente(s) de processamento
              </p>
            </div>
          </div>
              ...
              <div key={w.id} className="p-4 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-lg font-black text-emerald-400 shrink-0">
                    R${" "}
                    {parseFloat(w.amount).toLocaleString("pt-BR", {
                      minimumFractionDigits: 2,
                    })}
                  </span>
                </div>
                <div className="flex items-center gap-2 text-[11px] text-gray-400">
                  <code className="bg-gray-50 px-2 py-1 rounded-lg text-emerald-500 text-[11px] font-mono border border-gray-100 truncate">
                    {w.pix_key}
                  </code>
                  <button
                    onClick={() => navigator.clipboard.writeText(w.pix_key)}
                    className="p-1.5 bg-gray-100 rounded-lg text-gray-400 shrink-0"
                  >
                    <Save size={10} />
                  </button>
                ...
                <div className="flex items-center gap-2">
                  <input
                    id={`tx-m-${w.id}`}
                    placeholder="Hash"
                    className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-emerald-500/30 font-mono text-gray-900"
                  />
                  <button
                    onClick={() =>
                      handleAction("complete_withdraw", {
                        withdraw_id: w.id,
                        tx_hash: document.getElementById(`tx-m-${w.id}`).value,
                      })
                    }
                    className="bg-emerald-500 text-black px-3 py-2 rounded-lg font-bold text-xs shrink-0"
                  >
                    PAGO
                  </button>
                  <button
                    onClick={() =>
                      handleAction("reject_withdraw", { withdraw_id: w.id })
                    }
                    className="bg-red-500/10 text-red-500 px-3 py-2 rounded-lg font-bold text-xs border border-red-500/20 shrink-0"
                  >
                    NEGAR
                  </button>
                </div>
                      ...
                      <div className="flex justify-end items-center gap-3">
                        ...
                        <button
                          onClick={() =>
                            handleAction("complete_withdraw", {
                              withdraw_id: w.id,
                              tx_hash: document.getElementById(`tx-${w.id}`)
                                .value,
                            })
                          }
        ...
        {showFakeWithdrawModal && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-6">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => setShowFakeWithdrawModal(null)}
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-white border border-gray-200 shadow-2xl p-10 rounded-[40px] text-center"
            >
              <AlertTriangle
              ...
              <p className="text-gray-400 text-sm mb-8 italic">
                Aparecerá como{" "}
                <span className="text-gray-900 font-bold">PAGO</span> no
                histórico de{" "}
                <span className="text-gray-700">
                  {showFakeWithdrawModal.name}
                </span>
                .
              </p>
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleAction("create_fake_withdrawal", {
                    user_id: showFakeWithdrawModal.userId,
              ...
              >
                ...
                <button
                  type="submit"
                  className="w-full py-5 bg-primary text-white rounded-full font-black text-lg hover:bg-primary/90 transition-all"
                >
                  LANÇAR NO HISTÓRICO
                </button>
              </form>
