...
const STATUS_TABS = [
    ...
    { key: 'all',      label: 'Todos',      color: 'text-gray-500',   bg: 'bg-primary',    dot: 'bg-gray-300' },
];
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-white border border-gray-200 shadow-xl rounded-2xl w-full max-w-md">
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    ...
                    <button onClick={onClose} className="p-2 text-gray-400 hover:text-gray-900 rounded-lg hover:bg-gray-100 transition-all">
                        <X size={16} />
                    </button>
                </div>
                <div className="p-5 space-y-4">
                    <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">por {product.seller_name}</p>
                    </div>
                    {!isPending && (
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder:text-gray-300 focus:outline-none focus:border-primary/30 resize-none"
                        />
                    ...
                    <label className="flex items-center gap-3 cursor-pointer p-3 bg-gray-50 rounded-xl border border-gray-100 hover:bg-gray-100 transition-colors">
                        ...
                        />
                        <div>
                            <p className="text-sm font-bold text-gray-800">Enviar via Chat</p>
                            <p className="text-xs text-gray-500">A mensagem chegará na página de chats do vendedor</p>
                        </div>
                    </label>
                    <div className="flex gap-3">
                        <button onClick={onClose} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-500 hover:bg-gray-50 transition-all text-sm font-semibold">
                            Cancelar
                        </button>
...
function ProductDetailModal({ product, onClose, onApprove, onReject, onDelete }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-white border border-gray-200 shadow-xl rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between p-5 border-b border-gray-100">
                    <h2 className="font-black flex items-center gap-2 text-gray-900">
                ...
                <div className="p-5 space-y-5">
                    {/* Image */}
                    {product.image_url ? (
                        <img src={product.image_url} alt={product.name} className="w-full h-52 object-cover rounded-xl border border-gray-200" onError={e => e.target.style.display = 'none'} />
                    ) : (
                        <div className="w-full h-52 bg-gray-50 rounded-xl border border-gray-200 flex items-center justify-center">
                    ...
                    {product.delivery_info && (
                        <div>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Informações de Entrega</p>
                            <p className="text-sm text-gray-600 bg-gray-50 border border-gray-100 rounded-xl p-3 leading-relaxed">
                                {product.delivery_info}
                            </p>
                        </div>
                    ...
                    <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 flex items-center gap-3">
                        <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center flex-shrink-0">
                            <span className="font-black text-sm text-primary">{product.seller_name?.charAt(0).toUpperCase()}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                            <p className="font-bold text-sm text-gray-900">{product.seller_name}</p>
                            <p className="text-xs text-gray-400 truncate">{product.seller_email}</p>
                        </div>
                    ...
                    <div className="flex gap-2 pt-2 flex-wrap">
                        ...
                        {(product.status === 'pending' || product.vitrine == 0) && (
                            <button
                                ...
                                className="flex-1 py-3 rounded-xl bg-primary text-white font-black text-sm hover:bg-primary/90 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                            >
        ...
        <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Aprove ou reprove produtos submetidos pelos vendedores</p>
                </div>
                <button onClick={fetchProducts} className="p-2.5 bg-gray-100 border border-gray-200 rounded-xl text-gray-500 hover:text-gray-900 hover:bg-gray-200 transition-all">
                    <RefreshCw size={16} />
                </button>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    ...
                    { label: 'Total',      value: stats.total    || 0, color: 'text-gray-900',   border: 'border-gray-200',      bg: 'bg-white' },
                ].map(s => (
                ...
                <div className="flex gap-2">
                    {STATUS_TABS.map(tab => (
                        <button
                            ...
                            className={`relative px-4 py-2.5 rounded-xl text-xs font-bold transition-all ${status === tab.key ? 'bg-primary text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200 hover:text-gray-900'}`}
                        >
                            {tab.label}
            ...
            {loading ? (
                <div className="space-y-3">
                    {Array.from({ length: 5 }).map((_, i) => (
                        <div key={i} className="bg-gray-100 border border-gray-200 rounded-2xl h-24 animate-pulse" />
                    ))}
                </div>
            ) : products.length === 0 ? (
                ...
                <div className="space-y-3">
                    {products.map(p => (
                        <div
                            key={p.id}
                            className="bg-white border border-gray-100 shadow-sm rounded-2xl hover:shadow-md hover:border-gray-200 transition-all overflow-hidden"
                        >
                            {/* Mobile Layout */}
                            <div className="md:hidden">
                                <div className="p-4 space-y-3">
                                    ...
                                    <div className="flex items-start gap-3">
                                        {p.image_url ? (
                                            <img src={p.image_url} alt={p.name} className="w-12 h-12 rounded-xl object-cover flex-shrink-0 border border-gray-200" onError={e => e.target.style.display='none'} />
                                        ) : (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-2 mb-0.5">
                                                <h4 className="text-[14px] font-bold text-gray-900 truncate flex-1">{p.name}</h4>
                                                <StatusBadge status={p.status} />
                                            </div>
                                            <p className="text-[11px] text-gray-400"><User size={10} className="inline mr-1" />{p.seller_name}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        <button onClick={() => setDetail(p)} className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-gray-100 rounded-xl text-gray-500 text-[10px] font-bold active:scale-95 transition-transform">
                                            <Eye size={12} /> Ver
                                        </button>
                            ...
                            <div className="hidden md:block p-4">
                                <div className="flex items-center gap-4">
                                    {p.image_url ? (
                                        <img src={p.image_url} alt={p.name} className="w-16 h-16 rounded-xl object-cover flex-shrink-0 border border-gray-200" onError={e => e.target.style.display='none'} />
                                    ) : (
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start gap-3 mb-1">
                                            <p className="font-bold text-sm flex-1 truncate text-gray-900">{p.name}</p>
                                            <StatusBadge status={p.status} />
                                        </div>
                                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1">
                                            <span className="flex items-center gap-1.5 text-xs text-gray-400">
                                                ...
                                                <span className="text-gray-400">{p.seller_email}</span>
                                            </span>
                                            <span className="flex items-center gap-1 text-xs text-gray-400"><Tag size={11} /> {p.category}</span>
                                    ...
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        <button onClick={() => setDetail(p)} className="px-3 py-2 bg-gray-100 border border-gray-200 rounded-xl text-gray-500 hover:text-gray-900 hover:bg-gray-200 transition-all text-xs font-semibold flex items-center gap-1.5"><Eye size={13} /> Ver</button>
                                        {(p.status === 'pending' || p.vitrine == 0) && (
