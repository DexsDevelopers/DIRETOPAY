...
try {
    ...
    // Recent withdrawals
    $stmt = $pdo->query("SELECT id, user_id, amount_gross, amount, fee_platform, fee_gateway, pix_key, status, nominal, type, description, created_at FROM withdrawals ORDER BY id DESC LIMIT 25");
    ...
    // Recent transactions
    $stmtTx = $pdo->query("SELECT id, user_id, amount_brl, amount_net_brl, status, created_at FROM transactions ORDER BY id DESC LIMIT 20");
