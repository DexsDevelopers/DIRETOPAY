...
try {
    ...
    if ($count == 0) {
        $defaults = [
            ['account_approved', 'Sua conta DiretoPay foi APROVADA! 🔥', '<p>Olá, <strong>{name}</strong>!</p><p>Temos ótimas notícias: sua conta foi verificada e <strong>aprovada</strong> pela nossa equipe.</p><p>Você já pode acessar seu painel e começar a receber pagamentos com total blindagem e anonimato.</p>'],
            ...
            ['withdrawal_paid', '💸 Seu saque foi PAGO!', '<p>Olá, <strong>{name}</strong>!</p><p>Seu pedido de saque no valor de <strong>R$ {amount}</strong> foi processado e enviado com sucesso para sua chave PIX cadastrada.</p><p>Obrigado por utilizar o sistema DiretoPay!</p>'],
            ['global_announcement', '📢 Comunicado DiretoPay: {title}', '<p>Olá, <strong>{name}</strong>!</p><div style=\'padding: 15px; background: rgba(74, 222, 128, 0.05); border-left: 4px solid #4ade80;\'>{message}</div>']
        ];
...
<head>
    ...
    <title>DiretoPay - Configurações de E-mail</title>
